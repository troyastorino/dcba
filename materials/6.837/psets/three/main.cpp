...
namespace
{
    ...
    void initSystem(int argc, char * argv[])
    {
        ...
        if (argc < 2) {
            cout << "Please specify an integrator by passing 'e', 't', or 'r' as the first parameter to this script" << endl;
            exit(EXIT_FAILURE);
        }

        // read in the arguments
        char * integrator = argv[1];
        if (strcmp(integrator, "r") == 0) {
            // use the RK4 stepper
            timeStepper = new RK4();
        }
        else if (strcmp(integrator, "e") == 0) {
            timeStepper = new ForwardEuler();
        }
        else if (strcmp(integrator, "t") == 0) {
        ...
        pendulumSystem = new PendulumSystem(4);
        clothSystem = new ClothSystem(10, 6);
        systems.push_back(simpleSystem);
        systems.push_back(pendulumSystem);
        systems.push_back(clothSystem);
    ...
    {
        if(timeStepper!=0){
            timeStepper->takeStep(displayedSystem, timeStep);
        }
    }
            ...
            case 't':
                {
                    ...
                    displayedSystem = systems[systemIndex];
                    break;
                }
            case 'b':
                {
                    // adds random velocity to the cloth system
                    displayedSystem->addMotion();
                    break;
                }
            case ' ':
                {
